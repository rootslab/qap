/*
 * An ASCII string parser, intended for pattern strings less than 255 chars/bytes;
    ...
    var lookupTable = function ( p ) {
        ...
        QuickAsciiParser = function ( pattern ) {
            ...
            me.setPattern = function ( p ) {
                if ( p.length > 254 ) {
                    throw new Error( 'pattern length (' + p.length + ')\
                        should be less than 255 chars/bytes' );
                }
                me.p = p;
                me.plen = p.length;
